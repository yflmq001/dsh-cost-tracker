...
![dsh-cost-tracker：会话成本条与 /cost 命令](assets/screenshots/hero.png)
...
- **缓存写入按未命中价计，不按命中价计。** 提供方按「缓存未命中输入档」收取写入费用（如 DeepSeek 未命中 $0.14/M，命中仅 $0.0028/M）——把写入算作命中会在写入新上下文时低估约 50 倍。部分成本插件把缓存写入并入命中桶，本插件不这么做。
- **未知模型绝不静默计价。** 没有价格条目的模型会以「未配置」占位符出现在投影与 UI 中，而不是悄悄套用默认价——账单里藏不进猜测价。要么补价，要么看见缺口。
